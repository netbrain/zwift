# NixOS module for FHS based Zwift
{ zwift-package }:
{
  config,
  lib,
...
in
{
  options.programs.zwift = {
    enable = mkEnableOption "Zwift on Linux (FHS)";
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."zwift/config".text = ''
      ...
      ${lib.optionalString (
        cfg.zwiftWorkoutDir != ""
      ) "export ZWIFT_WORKOUT_DIR=\"${cfg.zwiftWorkoutDir}\""}
      ${lib.optionalString (
        cfg.zwiftActivityDir != ""
      ) "export ZWIFT_ACTIVITY_DIR=\"${cfg.zwiftActivityDir}\""}
      ${lib.optionalString (cfg.zwiftLogDir != "") "export ZWIFT_LOG_DIR=\"${cfg.zwiftLogDir}\""}
      ${lib.optionalString (
        cfg.zwiftScreenshotsDir != ""
      ) "export ZWIFT_SCREENSHOTS_DIR=\"${cfg.zwiftScreenshotsDir}\""}
      ${lib.optionalString cfg.zwiftOverrideGraphics "export ZWIFT_OVERRIDE_GRAPHICS=1"}
      ${lib.optionalString (
        cfg.zwiftOverrideResolution != ""
      ) "export ZWIFT_OVERRIDE_RESOLUTION=\"${cfg.zwiftOverrideResolution}\""}
