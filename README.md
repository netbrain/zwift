...
> :warning: **Before using ZWIFT_OVERRIDE_GRAPHICS**: This option requires that the `~/.config/zwift/graphics.txt` file exists. Make sure to launch zwift at least once so it creates the `graphics.txt` file or manually create it with the desired settings before using the `ZWIFT_OVERRIDE_GRAPHICS` option.
